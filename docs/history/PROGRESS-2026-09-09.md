...
```
adb -s <serial> shell am instrument -w -e class dev.androidagent.app.A11yToolsHardwareTest dev.androidagent.app.dev.test/androidx.test.runner.AndroidJUnitRunner
...
- PASS ON HARDWARE (2026-09-09): installed the dev debug APK with `adb install
  -r` on the designated Xiaomi test phone (`2201116TG`, Android 13). With the display timeout
