...
class AgentCoordinatorTest {
    @Test fun stopRevokesBeforeWaitingForEngineAndBlocksAnActionWaitingForOverlay() = runTest {
        ...
        runCurrent()
        rig.engine.emit(EngineEvent.ToolCall("1", "tap", buildJsonObject {}))
        runCurrent()
    ...
    @Test fun uiControlWaitsForOverlayAndBackendReadsDoNotShowIt() = runTest {
        ...
        runCurrent()
        rig.engine.emit(EngineEvent.ToolCall("read", "read_ui", buildJsonObject {}))
        runCurrent()
        assertEquals(0, rig.overlay.shown)
        rig.engine.emit(EngineEvent.ToolCall("tap", "tap", buildJsonObject {}))
        runCurrent()
